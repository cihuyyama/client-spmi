...
import { Indicator, JenisRekening, MataAnggaran } from "@/lib/types";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";

export default function DetailMataAnggaranContent({ id }: { id: string }) {
    ...
    function onSubmit(values: z.infer<typeof formSchema>) {
        try {
            toast.promise(
                axios.put(`${BASE_URL}/mata-anggaran/${id}`, values, {
    ...
    useEffect(() => {
        ...
        const fetchDataMataAnggaran = async () => {
            try {

                const response = await axios.get(`${BASE_URL}/mata-anggaran/${id}`, {
                    withCredentials: true
                })
                if (response.status === 200) {
                    setMa(response.data.data)
                    form.setValue('maCode', response.data.data.maCode)
                    form.setValue('name', response.data.data.name)
                    form.setValue('indicatorId', response.data.data.indicatorId)
                }
                return response.data;
            } catch (error) {
                return error;
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                ...
                                                <FormControl>
                                                    <Input placeholder="0.0" {...field} value={field.value} defaultValue={ma?.maCode} />
                                                </FormControl>
                                                <FormMessage />
                                            </FormItem>
                                        )}
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                ...
                                                <FormControl>
                                                    <Input placeholder="nama" {...field} value={field.value} defaultValue={ma?.name} />
                                                </FormControl>
                                                <FormMessage />
                                            </FormItem>
                                        )}
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                ...
                                                <Select onValueChange={field.onChange} value={field.value} defaultValue={ma?.indicatorId}>
                                                    <FormControl>
                                                        <SelectTrigger>
                                                    ...
                                                    <SelectContent>
                                                        {data && data.map((data: {id: string, name: string}) => (
