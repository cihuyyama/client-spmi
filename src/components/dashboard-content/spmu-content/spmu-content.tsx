...
import { JadwalPencairan, SPMU } from "@/lib/types";
import { useRouter, useSearchParams } from "next/navigation";
import { promise } from "zod";

export default function SPMUContent() {
  const { userInfo } = useSelector((state: RootState) => state.auth);
  const [data, setData] = useState<SPMU[]>([]);
    ...
    const fetchData = async () => {
      try {
        const [spp] = await Promise.all([
          axios.get(`${BASE_URL}/spmu?year=${selectedYear}&unitId=${selectedUnit}&periode=${selectedPeriode}`, {
            withCredentials: true
          })
        ])
        if (spp.status === 200) {
          setData(spp.data.data);
          router.replace(`${window.location.protocol}//${window.location.host}` + '/spmu?year=' + selectedYear + '&unitId=' + selectedUnit);
        }
      } catch (error) {
        return error;
      }
    ...
    const fetchDataReview = async () => {
      try {
        ...
        if (response.status === 200) {
          ...
          router.replace(`${window.location.protocol}//${window.location.host}` + '/spmu?year=' + selectedYear + '&unitId=' + selectedUnit + '&periode=' + selectedPeriode);
        }
      } catch (error) {
        console.error(error);
      }
