...
export default function DetailVisiMisiContent({ id }: { id: string }) {
    const { userInfo } = useSelector((state: RootState) => state.auth)
    const [vm, setVm] = useState<z.infer<typeof formSchema>>();
    ...
    const form = useForm<z.infer<typeof formSchema>>({
        ...
        defaultValues: {
            subUnitId: vm?.subUnitId ?? "none",
            unitId: vm?.unitId ?? "none",
            tahun: vm?.tahun ?? "",
            visi: vm?.visi ?? "",
            misi: vm?.misi ?? "",
            tujuan: vm?.tujuan ?? "",
    ...
    function onSubmit(values: z.infer<typeof formSchema>) {
        try {
            ...
            if (values.unitId !== "none" && values.subUnitId !== "none") {
                return toast.error("hanya salah satu Unit atau Sub Unit yang diisi")
            }
            ...
            toast.promise(
                axios.put(`${BASE_URL}/visi-misi/${id}`, values, {
    ...
    useEffect(() => {
        const fetchDataRole = async () => {
            try {
                const response = await axios.get(`${BASE_URL}/visi-misi/${id}`, {
                    withCredentials: true
                })
                if (response.status === 200) {
                    setVm(response.data.data)
                    form.setValue("subUnitId", response.data.data.subUnitId ?? "none")
                    form.setValue("unitId", response.data.data.unitId ?? "none")
                    form.setValue("tahun", response.data.data.tahun)
                    form.setValue("visi", response.data.data.visi)
                    form.setValue("misi", response.data.data.misi)
                    form.setValue("tujuan", response.data.data.tujuan)
                    form.setValue("keterangan", response.data.data.keterangan)
                }
            } catch (error) {
                return error;
            }
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                ...
                                                <Select onValueChange={field.onChange} defaultValue={vm?.unitId ?? "none"} value={field.value ?? "none"} >
                                                    <FormControl>
                                                        <SelectTrigger>
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                ...
                                                <Select onValueChange={field.onChange} defaultValue={vm?.subUnitId ?? "none"} value={field.value ?? "none"} >
                                                    <FormControl>
                                                        <SelectTrigger>
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                ...
                                                <Select onValueChange={field.onChange} defaultValue={vm?.tahun} value={field.value}>
                                                    <FormControl>
                                                        <SelectTrigger>
