...
import { MatoIndicator, SPP } from "@/lib/types";
import { useRouter, useSearchParams } from "next/navigation";

export default function EntryProgresContent() {
    ...
    const fetchData = async () => {
      try {
        const [spp] = await Promise.all([
          axios.get(`${BASE_URL}/spmu/recomended?year=${selectedYear}&unitId=${selectedUnit}`, {
            withCredentials: true
          })
        ])
        if (spp.status === 200) {
          setData(spp.data.data);
          // router.replace(`${window.location.protocol}//${window.location.host}` + '/spj/progres?year=' + selectedYear + '&unitId=' + selectedUnit);
        }
      } catch (error) {
        return error;
      }
